...
private
  ...
  def fetch_books
    ...
    if params[:sSearch].present?
      books = books.where("lower(title) like :search or lower(course_num) like :search or lower(dept) like :search", search: "%#{params[:sSearch].downcase}%")
    end
    books
  end
  ...
  def sort_column
    columns = %w[dept course_num title]
